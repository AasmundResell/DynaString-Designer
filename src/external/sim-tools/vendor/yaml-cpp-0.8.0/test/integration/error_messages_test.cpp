...
#define EXPECT_THROW_RUNTIME_ERROR(exception_type, statement, message) \
...
TEST(ErrorMessageTest, BadSubscriptErrorMessage) {
  ...
  EXPECT_THROW_RUNTIME_ERROR(YAML::BadSubscript,
                             doc["first"]["second"]["fourth"],
                             "operator[] call on a scalar (key: \"fourth\")");

  EXPECT_THROW_RUNTIME_ERROR(YAML::BadSubscript, doc["first"]["second"][37],
  ...
  EXPECT_THROW_RUNTIME_ERROR(YAML::BadSubscript, doc["first"]["second"][Node()],
                             "operator[] call on a scalar");
}
...
TEST(ErrorMessageTest, Ex9_1_InvalidNodeErrorMessage) {
  ...
  // Test that printable key is part of error message
  EXPECT_THROW_RUNTIME_ERROR(YAML::InvalidNode,
                             doc["first"]["fourth"].as<int>(),
                             "invalid node; first invalid key: \"fourth\"");

  EXPECT_THROW_RUNTIME_ERROR(YAML::InvalidNode, doc["first"][37].as<int>(),
