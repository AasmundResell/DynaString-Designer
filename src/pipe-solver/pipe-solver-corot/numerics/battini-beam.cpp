...
Vec7 local_deformations(const Vec3 &x1, const Vec3 &x2, const Vec3 &X1, const Vec3 &X2, const Mat3 &U1,
                        const Mat3 &U2) {
    ...
    if (local_rot_norm_node_1 != 0.00) {
        Mat3 log_R_local_node_1 = (local_rot_norm_node_1 / (2.0 * sin(local_rot_norm_node_1))) *
                                  (R_local_node_1 - (R_local_node_1.transpose()));
    ...
    if (local_rot_norm_node_2 != 0.00) {
        Mat3 log_R_local_node_2 = (local_rot_norm_node_2 / (2.0 * sin(local_rot_norm_node_2))) *
                                  (R_local_node_2 - (R_local_node_2.transpose()));
...
Mat3 rotation_vector_inverse_T_s(const uint node_id, const Vec3 &x1, const Vec3 &x2, const Vec3 &X1, const Vec3 &X2,
    ...
    if (phi_norm > 0.0) {
        ...
        const Mat3 phi_tilde{{0.0, -disp_l[2], disp_l[1]}, {disp_l[2], 0.0, -disp_l[0]}, {-disp_l[1], disp_l[0], 0.0}};

        T_inv = ((phi_norm / 2.0) / (tan(phi_norm / 2.0))) * Mat3::Identity();
        T_inv += (1.0 - ((phi_norm / 2.0) / (tan(phi_norm / 2.0)))) * u_vec * u_vec.transpose();
        T_inv -= 0.5 * phi_tilde;
    }
    // assert(is_orthonormal(T_inv, 1e-3));
