...
Vec12 calc_element_inner_forces(const uint ie, const vector<Vec3> &X, const vector<Vec3> &d_trans,
                                const vector<Quaternion> &d_rot, const scalar youngs, const scalar G, const scalar I_2,
                                const scalar I_3, const scalar A, const scalar J, const scalar beta,
...
inline void calc_element_kinematics(const uint ie, const vector<Vec3> &X, const vector<Vec3> &d_trans,
    ...
    assert(is_orthonormal(DeltaR_m));
    R_ = DeltaR_m * T;
    ...
    assert(is_orthonormal(E, 1.0E-4));
}
...
inline Mat7_12 calc_F_transpose(const Mat3 &T, const Mat3 &U, const Mat3 &E, const Mat3 &R_, const Vec7 &d_l,
    ...
    constexpr uint f6 = 2;
    constexpr uint f7 = 3;
    constexpr uint f10 = 4;
    constexpr uint f11 = 5;
    constexpr uint f12 = 6;
