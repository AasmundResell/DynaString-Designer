...
Vec12 calc_element_inner_forces(const uint ie, const vector<Vec3> &X, const vector<Vec3> &d_trans,
    ...
    if (beta > 0) {
        const Vec12 R_damp = calc_approx_rayleigh_beta_damping(beta, A, I_2, I_3, J, l0, youngs, G, E, U, T,
...
Vec7 calc_element_forces_local(const scalar l0, const scalar youngs, const scalar G, const scalar I_2, const scalar I_3,
    ...
    */
    ...
    Ip_e = K and these terms disappear.
    Governing equation is then:
    Ip_e * rho * phitors_tt = G * K * phitors_xx
    --------------------------------------------------------------------*/
